#!/bin/bash
# v2 部署：拉代码 → 备份 → 构建（不停服）→ 停机 → schema 升级 → 重启 → 验证
set -e
cd /opt/rag
...
echo "== 2/8 db backup =="
mkdir -p /opt/rag/backup
BK=/opt/rag/backup/rag_kb_pre_v2_$(date +%Y%m%d_%H%M%S).sql
pg_dump -h 127.0.0.1 -U rag_app -d rag_kb -f "$BK"
echo "backup: $BK ($(du -h "$BK" | cut -f1))"

echo "== 3/8 build java =="
cd /opt/rag/rag-java && mvn -q package -DskipTests
ls -lh target/rag-gateway-*.jar | tail -1

echo "== 4/8 build frontend =="
cd /opt/rag/rag-frontend && npm run build 2>&1 | tail -3

echo "== 5/8 stop services =="
cd /opt/rag
sudo systemctl stop rag-worker rag-python rag-qa rag-gateway
sleep 2

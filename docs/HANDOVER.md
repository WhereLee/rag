...
>
> **⛔ 过时声明（2026-09-01）**：本文为 2026-08-17 交付时快照，此后项目持续演进：4G 服务器生产部署与恢复演练、检索层并发优化（级联 rerank/线程调优）、LLM 切换 DeepSeek、COS 备份体系，并与 club-agent 集成（`/api/org/ingest`、`/api/org/retrieve`）。文中"应届毕业生"等用户背景也已过时（现为社招）。现状以 git log 与代码为准，本文保留为交付过程档案。
